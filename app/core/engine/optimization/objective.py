"""
Модуль целевой функции оптимизации (Objective Function).

Определяет класс `Objective`, который является "фитнес-функцией" для Optuna.
Его задача — принять набор параметров от оптимизатора, прогнать бэктест
на исторических данных (In-Sample) и вернуть числовую оценку эффективности (метрику).
"""

import optuna
import pandas as pd
import queue
import logging
from typing import Dict, List, Any, Type, Tuple, Union

from app.core.engine.backtest.loop import BacktestEngine
from app.core.risk.manager import AVAILABLE_RISK_MANAGERS
from app.core.analysis.metrics import PortfolioMetricsCalculator
from app.core.analysis.constants import METRIC_CONFIG
from app.core.calculations.indicators import FeatureEngine
from app.shared.config import config

EXCHANGE_SPECIFIC_CONFIG = config.EXCHANGE_SPECIFIC_CONFIG
BACKTEST_CONFIG = config.BACKTEST_CONFIG
PATH_CONFIG = config.PATH_CONFIG

logger = logging.getLogger(__name__)


class Objective:
    """
    Целевая функция для оптимизатора Optuna.

    Инкапсулирует логику одного "прогона" (Trial) оптимизации.
    Поддерживает оптимизацию как на одном инструменте, так и на портфеле.

    Attributes:
        strategy_class (Type): Класс стратегии для тестирования.
        exchange (str): Биржа.
        interval (str): Таймфрейм.
        risk_manager_type (str): Тип риск-менеджера.
        train_data_slices (Dict[str, pd.DataFrame]): Словарь с данными для обучения
            {Тикер: DataFrame}. Данные уже нарезаны `WFODataPreparer`.
        target_metrics (List[str]): Список метрик для оптимизации (например, ['calmar_ratio']).
        feature_engine (FeatureEngine): Сервис для расчета индикаторов.
    """

    def __init__(self,
                 strategy_class: Type,
                 exchange: str,
                 interval: str,
                 risk_manager_type: str,
                 train_data_slices: Dict[str, pd.DataFrame],
                 metrics: List[str],
                 feature_engine: FeatureEngine):
        """
        Инициализирует объект целевой функции.

        Args:
            strategy_class (Type): Класс стратегии.
            exchange (str): Название биржи.
            interval (str): Таймфрейм.
            risk_manager_type (str): Название риск-менеджера.
            train_data_slices (Dict): Данные для In-Sample теста.
            metrics (List[str]): Целевые метрики.
            feature_engine (FeatureEngine): Инстанс FeatureEngine.
        """
        self.strategy_class = strategy_class
        self.exchange = exchange
        self.interval = interval
        self.risk_manager_type = risk_manager_type
        self.train_data_slices = train_data_slices
        self.target_metrics = metrics
        self.instrument_list = list(train_data_slices.keys())

        self.annualization_factor = EXCHANGE_SPECIFIC_CONFIG[exchange]["SHARPE_ANNUALIZATION_FACTOR"]
        self.total_initial_capital = BACKTEST_CONFIG["INITIAL_CAPITAL"]
        self.feature_engine = feature_engine

    def _suggest_params(self, trial: optuna.Trial) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
        Генерирует значения параметров для текущей итерации (Trial).

        Сканирует `params_config` стратегии и риск-менеджера. Если параметр помечен
        как `optimizable=True`, запрашивает у Optuna значение в заданном диапазоне.
        Иначе использует значение по умолчанию.

        Args:
            trial (optuna.Trial): Объект текущей итерации Optuna.

        Returns:
            Tuple[Dict, Dict]: (Параметры стратегии, Параметры риск-менеджера).
        """
        strategy_params = {}
        strategy_full_config = {}

        # Собираем конфиг параметров со всех родительских классов (MRO)
        for base in reversed(self.strategy_class.__mro__):
            if hasattr(base, 'params_config'):
                strategy_full_config.update(base.params_config)

        for name, conf in strategy_full_config.items():
            if conf.get("optimizable", False):
                if conf["type"] == "int":
                    strategy_params[name] = trial.suggest_int(
                        name, conf["low"], conf["high"], step=conf.get("step", 1)
                    )
                elif conf["type"] == "float":
                    strategy_params[name] = trial.suggest_float(
                        name, conf["low"], conf["high"], step=conf.get("step")
                    )
            else:
                strategy_params[name] = conf["default"]

        # Аналогично для Риск-менеджера
        rm_class = AVAILABLE_RISK_MANAGERS[self.risk_manager_type]
        rm_params = {}
        rm_full_config = {}
        for base in reversed(rm_class.__mro__):
            if hasattr(base, 'params_config'):
                rm_full_config.update(base.params_config)

        for name, conf in rm_full_config.items():
            if conf.get("optimizable", False):
                # Добавляем префикс rm_, чтобы избежать коллизий имен с параметрами стратегии
                optuna_name = f"rm_{name}"
                if conf["type"] == "int":
                    rm_params[name] = trial.suggest_int(
                        optuna_name, conf["low"], conf["high"], step=conf.get("step", 1)
                    )
                elif conf["type"] == "float":
                    rm_params[name] = trial.suggest_float(
                        optuna_name, conf["low"], conf["high"], step=conf.get("step")
                    )
            else:
                rm_params[name] = conf["default"]

        return strategy_params, rm_params

    def __call__(self, trial: optuna.Trial) -> Union[float, Tuple[float, ...]]:
        """
        Выполняет оценку параметров.

        1. Генерирует параметры.
        2. Запускает `BacktestEngine` для каждого инструмента в `train_data_slices`.
        3. Собирает сделки со всех инструментов в единый DataFrame.
        4. Рассчитывает целевые метрики.

        Args:
            trial (optuna.Trial): Объект итерации.

        Returns:
            Union[float, Tuple[float, ...]]: Значение одной или нескольких целевых метрик.

        Raises:
            optuna.TrialPruned: Если сделок не было или расчет метрик невозможен.
        """
        try:
            strategy_params, rm_params = self._suggest_params(trial)
            all_instrument_trades = []

            # Распределяем капитал поровну между инструментами портфеля
            capital_per_instrument = self.total_initial_capital / len(self.instrument_list)

            for instrument, instrument_data_slice in self.train_data_slices.items():
                if instrument_data_slice.empty:
                    continue

                backtest_settings = {
                    "strategy_class": self.strategy_class,
                    "exchange": self.exchange,
                    "instrument": instrument,
                    "interval": self.interval,
                    "risk_manager_type": self.risk_manager_type,
                    "initial_capital": capital_per_instrument,
                    "commission_rate": BACKTEST_CONFIG["COMMISSION_RATE"],
                    "strategy_params": strategy_params,
                    "risk_manager_params": rm_params,
                    "data_slice": instrument_data_slice, # Передаем уже загруженные данные
                    "data_dir": PATH_CONFIG["DATA_DIR"]
                }

                events_queue = queue.Queue()

                # Создаем легковесный движок для этого прогона
                engine = BacktestEngine(
                    settings=backtest_settings,
                    events_queue=events_queue,
                    feature_engine=self.feature_engine
                )

                backtest_results = engine.run()

                if backtest_results["status"] == "success" and not backtest_results["trades_df"].empty:
                    all_instrument_trades.append(backtest_results["trades_df"])

            if not all_instrument_trades:
                # Если сделок нет, считаем этот набор параметров бесполезным
                raise optuna.TrialPruned("Ни на одном инструменте не было совершено сделок.")

            # Объединяем сделки по всем инструментам
            portfolio_trades_df = pd.concat(all_instrument_trades, ignore_index=True)
            portfolio_trades_df.sort_values(by='exit_timestamp_utc', inplace=True)

            # Рассчитываем метрики портфеля
            calculator = PortfolioMetricsCalculator(
                portfolio_trades_df,
                self.total_initial_capital,
                self.annualization_factor
            )

            if not calculator.is_valid:
                raise optuna.TrialPruned("Недостаточно сделок для расчета метрик.")

            all_calculated_metrics = calculator.calculate_all()

            # Сохраняем все метрики в атрибуты Trial (для последующего анализа)
            for metric_key, value in all_calculated_metrics.items():
                trial.set_user_attr(metric_key, value)

            # Возвращаем целевые метрики оптимизатору
            if len(self.target_metrics) == 1:
                return trial.user_attrs[self.target_metrics[0]]
            else:
                return tuple(trial.user_attrs[m] for m in self.target_metrics)

        except optuna.TrialPruned as e:
            # Штатная ситуация (плохие параметры), пробрасываем выше
            raise e
        except Exception:
            # Критическая ошибка в логике стратегии или движка
            logger.error(f"Критическая ошибка в trial #{trial.number}", exc_info=True)

            # Возвращаем "худшие" значения, чтобы оптимизатор ушел из этой области параметров
            if len(self.target_metrics) == 1:
                direction = METRIC_CONFIG[self.target_metrics[0]]['direction']
                return -1e9 if direction == 'maximize' else 1e9
            else:
                return tuple(
                    -1e9 if METRIC_CONFIG[m]['direction'] == 'maximize' else 1e9
                    for m in self.target_metrics
                )