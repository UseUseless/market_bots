...
Сканирование директории логов, чтение файлов сделок (`.jsonl`),
сопоставление их с историческими рыночными данными и расчет итоговых метрик
для отображения в сводной таблице Streamlit.
"""
...
def _process_single_backtest_file(file_path: str) -> Optional[Dict[str, Any]]:
    """
    Обрабатывает один файл лога сделок и рассчитывает метрики.

    Считывает JSONL файл, извлекает метаданные стратегии из первой строки,
    подгружает соответствующие исторические данные (если есть) и запускает
    `AnalysisSession` для расчета PnL, Шарпа и просадки.

    Args:
        file_path (str): Полный путь к файлу лога сделок (`..._trades.jsonl`).

    Returns:
        Optional[Dict[str, Any]]: Словарь с результатами анализа.
        Содержит ключи: 'Exchange', 'Strategy', 'PnL (Strategy %)', 'Sharpe Ratio' и др.
...
def load_all_backtests(logs_dir: str) -> Tuple[pd.DataFrame, List[str]]:
    """
    Сканирует директорию логов и собирает сводную таблицу результатов.

    Использует кэширование Streamlit (`@st.cache_data`) для ускорения повторных загрузок.
    Отображает прогресс-бар в интерфейсе во время обработки файлов.

    Args:
        logs_dir (str): Путь к директории, в которой нужно искать файлы `_trades.jsonl`.

    Returns:
        Tuple[pd.DataFrame, List[str]]: Кортеж из двух элементов:
            1. DataFrame: Сводная таблица всех успешно обработанных бэктестов.
            2. List[str]: Список сообщений об ошибках для файлов, которые не удалось загрузить.
    """
    all_results = []
    failed_files = []
