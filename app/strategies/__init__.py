...
стратегий из подпапки `app/strategies/_дирестория_со_стратегиями_ - catalog/`.
Чтобы добавить новую стратегию, достаточно создать файл в папке `catalog/`.
и унаследовать класс от `BaseStrategy` реализовав def _calculate_signals и атрибуты наследуемого класса.
...
def _discover_strategies() -> Dict[str, Type[BaseStrategy]]:
    ...
    Алгоритм работы:
    1. Сканирует директорию `app/strategies/_директория_со_стратегиями_`.
    ...
    Returns:
        Dict[str, Type[BaseStrategy]]: Словарь:ключ — имя файла (без .py),
        значение — ссылка на класс стратегии.
    """
    strategies_dict = {}
