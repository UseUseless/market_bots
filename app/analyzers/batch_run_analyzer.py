...
class BatchRunAnalyzer:
    ...
    def __init__(
        ...
        if results_df.empty:
            raise ValueError("DataFrame с результатами не может быть пустым.")
    ...
    def _calculate_summary_metrics(self) -> pd.DataFrame:
        """Рассчитывает сводные метрики по всему портфелю."""
    ...
    def generate_excel_report(self, output_path: str):
        ...
        print(f"Excel-отчет успешно сохранен в: {output_path}")