...
from app.core.stream.data_handler import TinkoffStreamDataHandler, BybitStreamDataHandler, BaseStreamDataHandler
...
def main():
    ...
    parser.add_argument("--params", type=str, default=None,
                        help="Параметры для стратегии и РМ в формате JSON строки.")
    ...
    if args.params:
        ...
        try:
            ...
            strategy_params_override = {k: v for k, v in all_params.items() if not k.startswith("rm_")}
            rm_params_override = {k[3:]: v for k, v in all_params.items() if k.startswith("rm_")}
        except json.JSONDecodeError:
            logging.error("Ошибка парсинга JSON в --params.")
            return
    ...
    final_rm_params.update(rm_params_override)
