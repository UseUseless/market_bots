...
def process_and_analyze_results(results: Dict[str, Any], config: Dict[str, Any]):
    """Обрабатывает и анализирует результаты, полученные от движка бэктеста."""
    if results["status"] != "success":
        logger.error(f"Бэктест завершился с ошибкой: {results.get('message')}")
        return
    ...
    if not trades_df.empty:
        ...
        report_filename = os.path.basename(config["trade_log_path"]).replace('_trades.jsonl', '')
...
def main():
    ...
    try:
        ...
        process_and_analyze_results(results, backtest_config)
